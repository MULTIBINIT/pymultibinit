...
class MultibinitPotential:
    ...
    def _prepare_for_evaluation(self, positions: np.ndarray, lattice: np.ndarray,
                                skip_atom_matching: bool = False) -> Tuple[np.ndarray, bool]:
        """Validate the structure and resolve atom matching.

        Shared by :meth:`evaluate` and :meth:`evaluate_contributions` so that
        both paths apply identical validation, atom reordering, and PBC shifts.

        Returns ``(positions_for_eval, need_force_remapping)`` where
        ``positions_for_eval`` is reordered to the internal reference order and
        ``need_force_remapping`` flags whether force arrays must later be mapped
        back to the input atom order.
        """
        if not self._initialized:
            ...
            if self._atom_mapping is None:
                try:
                    ...
                    if not self._is_identity_mapping:
                        if self._atom_mapping is not None and not np.array_equal(
                                self._atom_mapping, np.arange(len(positions))):
                            warnings.warn(
        ...
        return positions_for_eval, need_force_remapping

    def evaluate_contributions(self, positions: np.ndarray, lattice: np.ndarray,
                               skip_atom_matching: bool = False) -> dict:
        """
        Evaluate energy, forces, and stress decomposed by physical term.

        Returns a dict ``{term_name: (energy, forces, stress)}`` in ASE units:
        energy in eV, forces in eV/Angstrom, stress in eV/Angstrom^3 (Voigt
        order ``[xx, yy, zz, yz, xz, xy]``). Forces are returned in the input
        atom order, exactly like :meth:`evaluate`.

        Terms: ``reference``, ``harmonic_local`` (local harmonic IFCs),
        ``dipdip`` (dipole-dipole / long-range Coulomb), ``anharmonic``,
        and, when present, ``elastic`` and ``strain_coupling``. The arrays
        summed over all terms reproduce :meth:`evaluate` exactly.

        Only the pure-Python (``pyeffpot``) backend supports decomposition; the
        CFFI (Fortran) backend raises :class:`NotImplementedError`.
        """
        positions_for_eval, need_force_remapping = self._prepare_for_evaluation(
            positions, lattice, skip_atom_matching)
        ...
        if self.backend != 'pyeffpot' or self._pyeffpot_potential is None:
            raise NotImplementedError(
                "Energy decomposition is only supported by the pure-Python (pyeffpot) "
                "backend. The CFFI (Fortran) backend does not expose per-term contributions."
            )

        pos_bohr = positions_for_eval * ANGSTROM_TO_BOHR
        lat_bohr = lattice * ANGSTROM_TO_BOHR

        raw = self._pyeffpot_potential.evaluate_contributions(pos_bohr, lat_bohr.T)

        energy_factor = HARTREE_TO_EV
        force_factor = HARTREE_TO_EV / BOHR_TO_ANGSTROM
        stress_factor = HARTREE_TO_EV / (BOHR_TO_ANGSTROM ** 3)

        result: dict = {}
        for name, (e_ha, f_ha, s_ha) in raw.items():
            if need_force_remapping and self._inverse_mapping is not None:
                f_ha = apply_inverse_mapping_to_forces(f_ha, self._inverse_mapping)
            if np.asarray(s_ha).shape == (3, 3):
                s_voigt = np.array([
                    s_ha[0, 0], s_ha[1, 1], s_ha[2, 2],
                    s_ha[1, 2], s_ha[0, 2], s_ha[0, 1],
                ])
            else:
                s_voigt = np.asarray(s_ha, dtype=float)
            result[name] = (
                float(e_ha) * energy_factor,
                np.asarray(f_ha, dtype=float) * force_factor,
                np.asarray(s_voigt, dtype=float) * stress_factor,
            )
        return result
