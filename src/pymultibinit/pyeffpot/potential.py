...
class EffectivePotential:
    ...
    def evaluate(self, xcart: Optional[np.ndarray] = None,
        ...
        if hasattr(self.supercell, 'phonon_strain_sc') and self.supercell.phonon_strain_sc is not None:
            e_sc, f_sc, s_sc = self._evaluate_strain_coupling(strain, displacements)
            energy += e_sc
            forces += f_sc
            stress += s_sc
        ...
        if self.supercell.anharmonic_coeffs:
            e_anh, f_anh, s_anh = self._evaluate_anharmonic(displacements, strain)
            energy += e_anh
            forces += f_anh
            stress += s_anh
    ...
    def _evaluate_strain_coupling(self, strain: np.ndarray, 
        ...
        # TODO: The formula above gives stress in Voigt notation...
            ...
            for iz0 in range(nz):
                for iy0 in range(ny):
                    for ix0 in range(nx):
                        ...
                        i_sc = atom_uc + natom_uc * (ix + nx * (iy + ny * iz))
                        idx[count] = i_sc
                        count += 1
            return idx
    ...
    def _evaluate_anharmonic(self, displacements: np.ndarray, strain: np.ndarray) -> Tuple[float, np.ndarray, np.ndarray]:
        """
        Evaluate anharmonic polynomial terms from XML coefficients.
        
        Energy: E = Σ c_k Π (u)^n Π (ε)^m
        Forces: F = -∇_u E
        Stress: σ = 1/V * ∇_ε E
        ...
        forces : np.ndarray
            Anharmonic forces (natom_sc, 3).
        stress : np.ndarray
            Anharmonic stress (3, 3).
        """
        natom = self.supercell.natom_sc
        ...
        if compiled_terms is None:
            return energy, forces, np.zeros((3, 3))
        ...
        for term_info in compiled_terms:
            ...
            for disp in term_info['displacements']:
                if disp['power'] == 0:
                    continue
                diff = displacements[disp['idx_b'], disp['dir']] - displacements[disp['idx_a'], disp['dir']]
            ...
            for k, disp_k in enumerate(term_info['displacements']):
                ...
                for j, disp_j in enumerate(term_info['displacements']):
                    ...
                    if p_j == 0:
                        continue
                    diff_j = displacements[disp_j['idx_b'], disp_j['dir']] - displacements[disp_j['idx_a'], disp_j['dir']]
                    if j == k:
                        deriv *= diff_j ** (p_j - 1)
                    else:
                        deriv *= diff_j ** p_j
