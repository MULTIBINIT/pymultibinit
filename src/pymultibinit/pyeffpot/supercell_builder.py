...
def _build_supercell_ifcs_fourier(
    ...
    # The binary only computes dipdip when ncell >= ngqpt.
    # For ncell < ngqpt, the C-API does not adapt dipdip_range.
    dipdip_active = any(int(n) > int(g) for n, g in zip(ncell, ngqpt))
    has_dipdip = (dipdip and dipdip_active
    ...
    # Fold wghatm into the IFC values
    ...
    if has_dipdip:
        ...
        for irpt_u in range(nrpt_union):
            i1 = int(cell_union[0, irpt_u]); i2 = int(cell_union[1, irpt_u]); i3 = int(cell_union[2, irpt_u])
            if not (sc_min[0] <= i1 <= sc_max[0] and sc_min[1] <= i2 <= sc_max[1] and sc_min[2] <= i3 <= sc_max[2]):
                continue
